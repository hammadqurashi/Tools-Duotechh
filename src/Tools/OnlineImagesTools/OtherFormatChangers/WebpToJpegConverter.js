...
const ImageFormatChanger = () => {
  ...
  const addFile = async () => {
    ...
    if (!file) {
      ...
      var isValid = /\.webp$/i.test(uploadRef.current.value);
      if (isValid) {
  ...
  const convert = (key) => {
    ...
    image.onload = async () => {
      ...
      for (let index = 0; index < convertImg.length; index++) {
        ...
        if (element.key === key) {
          formatChange = await canvas.toDataURL(`image/jpeg`, 0.7);
  ...
  return (
    <div className="otherFormatChangers_Container">
      <h1 className="tool_h1">WEBP TO JPEG CONVERTER</h1>
      <div className="otherFormatChanger_Box">
        ...
        <button id="ofc-addFiles" onClick={() => uploadRef.current.click()}>
          <img
            ...
            id="ofc-addFiles_img"
            src={require("../../../Assets/OtherImages/webp.webp")}
            alt="Upload"
          />
